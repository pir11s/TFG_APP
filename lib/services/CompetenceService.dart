...
class CompetenceService {
  ...
  static readData() async {
    final String responseLeaders =
        await rootBundle.loadString('assets/json/CompetenceLeaders.json');
    CompetencesLeaders competencesLeaders =
        CompetencesLeaders.fromJson(jsonDecode(responseLeaders));
    ...
    final String responseDetails =
        await rootBundle.loadString('assets/json/CompetenceDetails.json');
    CompetencesDetails competencesDetails =
        CompetencesDetails.fromJson(jsonDecode(responseDetails));
    ...
    final String responseTechnologies =
        await rootBundle.loadString('assets/json/EmployeesTech.json');
    EmployeesTechnologies employeesTechnologies =
        EmployeesTechnologies.fromJson(jsonDecode(responseTechnologies));
    for (EmployeeTechnology e in employeesTechnologies.employeesTechnologies!) {
      competences[e.competenceName]!.members[e.employeeName!] =
          e.employeeName!.toUpperCase();
      competences[e.competenceName]!.technologiesSkillLevel[e.technology!] =
          e.skillLevel!;
    }

    final String responseIndustryRelevance =
        await rootBundle.loadString('assets/json/CompetenceTechnologies.json');
    CompetencesTechnologies competencesTechnologies =
        CompetencesTechnologies.fromJson(jsonDecode(responseIndustryRelevance));
    for (Competences e in competencesTechnologies.competences!) {
      competences[e.competenceName]!
              .technologiesIndustryRelevance[e.technology!] =
          e.industryRelevance != null ? e.industryRelevance! : "1";
    }
  }
